...
final class phpBitcoinSignature {
  ...
  public function isMessageSignatureValid($address, $signature, $message) {
	  ...
	  // Doubly hash the message and recover key
	  try {
	    $messageHash = hash('sha256',
	                        hash('sha256',
	    ...
	    $pubkey = $this->recoverPubKey($this->bin2gmp(substr($signature, 1, 32)),
	                                   $this->bin2gmp(substr($signature, 33, 32)),
	                                   $this->bin2gmp($messageHash),
	                                   $recoveryFlags,
	                                   $secp256k1_G);
	    ...
	    // Check that the key we recovered is for the address given
	    if (!$isCompressed) {
  ...
  public function isBignumEven($bnStr) {
  ...
  public function recoverPubKey($r, $s, $e, $recoveryFlags, $G) {
	  ...
	  try {
	    ...
	    $alpha = gmp_mod(gmp_add(gmp_add(gmp_pow($x, 3),
  ...
  public function bin2gmp($binStr) {
    try {
	    $v = gmp_init('0');
